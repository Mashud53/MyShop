...
const LaptopDell = () => {
    ...
    useEffect(() => {
        if(allProducts.length>0){
            const dell = allProducts.filter(item => item.brand == 'Dell' && item.category == "Laptop")
        setDellProduct(dell)
        }
    }, [allProducts])
            ...
            
            {
                dellProduct?.length > 0 ?
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center font-catamaran text-base font-semibold  md:pt-10 mx-auto">
                        {
                            dellProduct.map(products => <ProductCard key={products._id} products={products}></ProductCard>)
                        }
                    </div> :
