...
const MobileXiaomi = () => {
    ...
    const [xiaomiProduct, setXiaomiProduct] = useState([]);

    useEffect(() => {
        if (allProducts.length>0) {
            const xiaomi = allProducts.filter(item => item.brand == 'Xiaomi' && item.category == "Mobile")
            setXiaomiProduct(xiaomi)
        }
    }, [allProducts])
...

            {
                xiaomiProduct?.length > 0 ?
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center font-catamaran text-base font-semibold  md:pt-10 mx-auto">
                        {
                            xiaomiProduct.map(products => <ProductCard key={products._id} products={products}></ProductCard>)
                        }
                    </div> :
