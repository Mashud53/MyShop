...
const MobileOnePlus = () => {
    ...
    const [onePlusProduct, setOnePlusProduct] = useState([]);

    useEffect(() => {
        if(allProducts.length>0){
            const onePlus = allProducts.filter(item => item.brand == 'One Plus' && item.category == "Mobile")
        setOnePlusProduct(onePlus)
        }
    }, [allProducts])
            ...
            
            {
                onePlusProduct?.length > 0 ?
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center font-catamaran text-base font-semibold  md:pt-10 mx-auto">
                        {
                            onePlusProduct.map(products => <ProductCard key={products._id} products={products}></ProductCard>)
                        }
                    </div> :
