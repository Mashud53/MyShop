...
const MobileVivo = () => {
    ...
    const [vivoProduct, setVivoProduct] = useState([]);

    useEffect(() => {
        if(allProducts.length>0){
            const vivo = allProducts.filter(item => item.brand == 'Vivo' && item.category == "Mobile")
        setVivoProduct(vivo)
        }
    }, [allProducts])
            ...
            
            {
                vivoProduct?.length > 0 ?
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center font-catamaran text-base font-semibold  md:pt-10 mx-auto">
                        {
                            vivoProduct.map(products => <ProductCard key={products._id} products={products}></ProductCard>)
                        }
                    </div> :
