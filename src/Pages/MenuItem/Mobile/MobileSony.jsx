...
const MobileSony = () => {
    ...
    const [sonyProduct, setSonyProduct] = useState([]);

    useEffect(() => {
        if(allProducts.length>0){
            const sony = allProducts.filter(item => item.brand == 'Sony' && item.category == "Mobile")
        setSonyProduct(sony)
        }
    }, [allProducts])
            ...
            
            {
                sonyProduct?.length > 0 ?
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 justify-center items-center font-catamaran text-base font-semibold  md:pt-10 mx-auto">
                        {
                            sonyProduct.map(products => <ProductCard key={products._id} products={products}></ProductCard>)
                        }
                    </div> :
